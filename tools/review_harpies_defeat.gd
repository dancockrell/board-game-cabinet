...
const SOUTH=preload("res://themes/harpies_south_defeat.tres")
const NORTH=preload("res://themes/harpies_north_defeat.tres")
const SOUTH_REST=preload("res://themes/harpies_south_flight.tres")
const NORTH_REST=preload("res://themes/harpies_north_flight.tres")
...
func run():
    ...
    for i in 4:
        ...
        actor.position=Vector3(-2.1+i*1.4,1.0,3)
    ...
    for frame in 96:
        ...
        for i in 4:
            ...
            if i%2==0:
                ...
                var landing=.22 if i==1 else .31
                actor.position.y=1.0*(1.0-smoothstep(0,landing,elapsed))
        await process_frame
        await RenderingServer.frame_post_draw
        root.get_texture().get_image().save_png(output.path_join("frame-%03d.png"%frame))
    print("Harpy defeat study:96frames, flying rest beside south/north eight-pose ground-landing defeat; coordinator supplies runtime descent")
    quit(0)
