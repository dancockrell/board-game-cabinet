...
func run():
	...
	for frame in 90:
		if frame==15 or frame==51: actor.play_attack(frame,Attack)
		actor.advance_visual(1.0/60.0)
		await process_frame
		await RenderingServer.frame_post_draw
		root.get_texture().get_image().save_png(output.path_join("frame-%03d.png"%frame))
	print("Harpy south attack: 90 native frames at 60fps captured; flight, attack, recovery twice")
	quit(0)
