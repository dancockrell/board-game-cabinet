...
const REST = {
    "south": preload("res://themes/heracles_south_rest.tres"),
    "north": preload("res://themes/heracles_north_rest.tres"),
    "east": preload("res://themes/heracles_east_rest.tres"),
    "west": preload("res://themes/heracles_west_rest.tres")
}
const ATTACK = {
    "south": preload("res://themes/heracles_south_attack.tres"),
    "north": preload("res://themes/heracles_north_attack.tres"),
...
func _run() -> void:
    ...
    for facing in REST:
        if not REST[facing].validation_error().is_empty() or not ATTACK[facing].validation_error().is_empty():
            push_error("Invalid Heracles clip "+facing)
            quit(1); return
    ...
    for i in actors.size():
        if actors[i].clip!=REST[REST.keys()[i]]:
            push_error("Heracles failed to recover "+REST.keys()[i]); quit(1); return
    print("Heracles: 8 resource validations, 4 attack/recovery checks, 90 native frames; passed")
    quit(0)
