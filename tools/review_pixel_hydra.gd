extends SceneTree
const SouthIdle = preload("res://themes/hydra_south_idle.tres")
const NorthIdle = preload("res://themes/hydra_north_idle.tres")
const SouthAttack = preload("res://themes/hydra_south_attack.tres")
const NorthAttack = preload("res://themes/hydra_north_attack.tres")
...
func _run() -> void:
	...
	if "--validate-only" in OS.get_cmdline_user_args():
		print("Hydra: four clips valid, north/south idle and attack")
		quit(0); return
	...
	for i in 2:
		...
		actors.append(actor)
	var title:=Label.new()
	title.text="HYDRA / SOUTH AND NORTH / BREATHING AND BITE STUDY"
