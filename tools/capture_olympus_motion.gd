...
func _run() -> void:
	...
	for frame in warmup_frames:
		...
		if wait_for_thrust:
			for token in app.board._tokens.values():
				...
				if sprite != null and sprite.clip in [app.board.THRUST_CLIP,app.board.NORTH_ATTACK,app.board.SOUTH_ATTACK]: found_thrust=true
			if found_thrust:
	...
	for frame in CAPTURE_FRAMES:
		...
		for token in app.board._tokens.values():
			var sprite=token.get_node_or_null("Figure/PixelActor")
			if sprite != null and sprite.clip in [app.board.THRUST_CLIP,app.board.NORTH_ATTACK,app.board.SOUTH_ATTACK]: pixel_attack_frames+=1
