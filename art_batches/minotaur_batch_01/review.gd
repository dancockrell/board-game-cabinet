...
func _initialize() -> void:
	...
	for d in 4:
		...
		var path := "res://art_batches/minotaur_batch_01/"+direction+".png"
		...
		for row in 4:
			...
			sprites.append(s)
			var label := Label.new()
			label.text = direction+" / "+["walk", "charge", "attack", "defeat"][row]
...
func _step() -> void:
	...
	capturing = false
	if tick >= 96:
		print("Minotaur batch preview: 16 rows, 128 candidate cells, two playback cycles; not runtime admitted")
		quit()
