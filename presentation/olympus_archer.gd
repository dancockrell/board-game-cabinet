...
func _tube(parent: Node3D, points: Array, radii: Array, color: Color, segments := 10) -> MeshInstance3D:
	var typed_points: Array[Vector3] = []
	var typed_radii: Array[float] = []
	typed_points.assign(points)
	typed_radii.assign(radii)
	return sculpt.tube(parent,typed_points,typed_radii,color,segments)
...
func build(p: Node3D, team: Color) -> void:
	...
	for i in 13:
		var a:=i*TAU/13
		_tube(p,[Vector3(cos(a)*.148,.76,sin(a)*.107),Vector3(cos(a)*.175,.65,sin(a)*.128),Vector3(cos(a)*.217,.548,sin(a)*.15)],[.006,.008,.010],green.darkened(.10+(i%3)*.025),7)
	# Cross-body leather baldric and a narrow team-colored cloth belt.
	_tube(p,[Vector3(-.13,1.09,.075),Vector3(-.065,.97,.117),Vector3(.02,.88,.114),Vector3(.113,.77,.067)],[.019,.019,.018,.017],leather,8)
	...
	for side in [-1,1]:
		...
		_tube(arm,[Vector3.ZERO,elbow,hand],[.051,.039,.028],skin,12)
		f.ball(arm,hand,Vector3(.035,.048,.034),skin)
		_tube(arm,[elbow.lerp(hand,.30),elbow.lerp(hand,.86)],[.037,.031],leather,12)
	...
	_tube(p,[Vector3(0,1.351,.069),Vector3(0,1.317,.084)],[.009,.012],skin,8)
	...
	for side in [-1,1]:
		for strand in 5:
			_tube(p,[Vector3(side*.014,1.437,-.042+strand*.014),Vector3(side*.075,1.392,-.034+strand*.012),Vector3(side*.074,1.31,-.046+strand*.01)],[.013,.014,.004],hair.lightened((strand%3)*.035),7)
	var pony:Node3D=f.joint(p,"Ponytail",Vector3(0,1.377,-.092))
	_tube(pony,[Vector3.ZERO,Vector3(.025,-.10,-.045),Vector3(.043,-.24,-.065),Vector3(.018,-.31,-.04)],[.044,.052,.032,.003],hair,12)
	...
	for i in 4:
		...
		_tube(quiver,[Vector3(x,.12,0),Vector3(x,.49,0)],[.004,.004],Color("b6a582"),6)
...
func _bow(arm:Node3D, hand:Vector3) -> void:
	...
	bow.position=hand
	bow.rotation.z=.13
	_tube(bow,[Vector3(0,-.50,-.01),Vector3(0,-.39,.07),Vector3(0,-.20,.105),Vector3.ZERO,Vector3(0,.20,.105),Vector3(0,.39,.07),Vector3(0,.50,-.01)],[.005,.012,.018,.021,.018,.012,.005],Color("927044"),10)
	_tube(bow,[Vector3(0,-.50,-.01),Vector3(0,0,-.14),Vector3(0,.50,-.01)],[.0025,.0025,.0025],Color("c6c1a9"),6)
	_tube(bow,[Vector3(0,0,-.16),Vector3(0,0,.46)],[.004,.004],Color("c0ad88"),6)
