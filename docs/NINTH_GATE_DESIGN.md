...
Design authority, 2026-09-05. Working title; original Heaven-versus-Hell fiction is provisional while the user chooses the mythology and battle. This document records the intended game and separates the first prototype from later acceptance work. “Best Kriegsspiel” is the design ambition, to be tested with players, not an established claim.
...
Design targets: learn the opening scenario in ten minutes; finish a battle in roughly 20–40 minutes; make a typical order-planning round take about a minute. These are playtest targets, not measured properties. Avoid long tables, arbitrary surprise exceptions, repeated confirmation dialogs, and a simulation layer that contributes no decision.

## The battle: Heaven and Hell

The provisional scenario is a siege at the Ninth Gate, a crossing before a celestial fortress. It is not yet an adaptation of Milton, Revelation, or a settled fictional canon. Its compact crossing map is a rules experiment and does not yet implement fortress siege mechanics. Heaven's ivory-and-gold blocks and Hell's charcoal-and-copper blocks represent formations on a physical map table. The battlefield needs a clear centre, constrained crossings, defensive terrain, and alternative approaches. The conflict is about controlling the crossing, not depicting real religious communities.

Use legible role symbols and original formation names. A player must understand a formation's job without learning a bestiary. The current experiment uses matched rosters with three explicit faction modifiers: Heaven's guards have extra cover, Heaven rallies morale more quickly, and Hell attacks harder. These express coordinated defence and aggressive commitment; they remain balance hypotheses. Archers have additional range, spears additional attack, and heralds currently follow ordinary unit rules. Later mobile formations and commander abilities need explicit costs and counters rather than unexplained exemptions.
...
Later reconnaissance can add uncertain last-seen contacts and report age, but the first release must not suggest such memory exists until implemented. Likewise, command distance, order delay, morale, retreat, supply, and faction abilities are later experiments; they are not implied by the word Kriegsspiel.
...
The first implementation is one compact scenario, readable counters, order planning and commitment, an identified local opponent, automatic adjudication, fog-filtered presentation, and an observable win/loss/draw result. The source now includes a simple 2D map scene and a theme Resource; the wooden 3D war-table treatment below is a later polish target. Exact implemented rules and controls belong in the handoff's prototype contract and executable tests. Native full-battle acceptance and balance playtesting remain open.
...
The first map should feel like an officer's wooden table: a framed terrain surface, inset routes and crossings, simple raised terrain, warm side lighting, and small branded military blocks. Use restrained gold and copper accents; retain shape and symbol differences in grayscale. Geography must remain readable beneath order arrows and selection marks. Fantasy belongs in the landscape and iconography before it becomes particles or characters.
...
After the baseline is readable and fun: add one alternate scenario; test command distance or morale independently; give factions a small asymmetry; then add paper-map, stone-table, and lacquered-counter sets. Animated Heaven/Hell characters are a later theme with the same formation IDs and legal state. Long battle animations must be skippable and cannot postpone the acceptance of authoritative results.
