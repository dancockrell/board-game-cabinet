...
### Authored runtime increment

The thrust source is now integrated provisionally using explicit unequal regions, individual foot pivots, a 1.22 draw-scale correction, and two source-coordinate exclusion rectangles to remove adjacent-frame fragments without clipping the thrust spear. Source remains unchanged. Four timings total 0.44 seconds; attack event IDs are deduplicated separately from damage reaction IDs. Native contact/draw/recovery stills reviewed at gameplay renderer scale. This supersedes the automatic-quarter-crop rejection only: final directional admission is still incomplete. One right-facing action currently plays for all Hoplite attacks, so attacks can face away from targets. Idle-to-windup transition and foot/outline consistency also need polish. Do not label this a finished directional combat set.
