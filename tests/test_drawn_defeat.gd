...
func run():
	for direction in ["north","south"]:
		...
		var state={"units":[{"id":7,"kind":"medusa","side":0,"x":0.0,"z":2.0,"hp":100.0,"max_hp":100.0}],"towers":[],"events":[],"elapsed":1.0}
		...
		check(departure.drawn_actor!=null,"Medusa uses drawn defeat")
		var actor=departure.drawn_actor
		check(actor.clip==Board.MEDUSA_DEFEAT[direction],"Facing chooses matching defeat drawings")
		...
		board.free()
	print("Drawn defeat board integration: two facings, eight frames each, pause, placement, removal and lifetime; failures: ",failures)
