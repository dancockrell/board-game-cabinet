extends SceneTree
const Walk = preload("res://themes/medusa_north_walk.tres")
var checks = 0
var failures = 0
...
func run():
	...
	if DisplayServer.get_name() != "headless":
		...
		for step in 24:
			...
			check(actor.clip == Walk, "Moving north chooses north gait")
		...
		state.elapsed += .05
		board.show_state(state,.05)
		check(actor.clip == board.MEDUSA_REST.north, "Stopped Medusa returns north rest")
		board.free()
	print("Medusa north walk: %d checks, %d failures" % [checks,failures])
