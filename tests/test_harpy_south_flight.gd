extends SceneTree
const Walk=preload("res://themes/harpies_south_flight.tres")
...
func run():
	...
	if DisplayServer.get_name()=="headless": print("Harpy south flight: eight distinct cells, ordered timing and loop passed"); quit(0); return
	...
	var state={"units":[{"id":1,"kind":"harpies","flying":true,"side":1,"x":0.0,"z":-2.0,"hp":100.0,"max_hp":100.0}],"towers":[],"elapsed":1.0,"events":[]}
	...
	state.elapsed+=.05
	board.show_state(state,.05)
	assert(moving.clip==board.HARPIES_FLIGHT.south)
	board.free()
	print("Harpy south flight: eight distinct cells, ordered timing, loop, pause and reset passed")
	quit(0)
