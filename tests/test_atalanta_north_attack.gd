...
func _initialize():
	...
	check(ATTACK.frame_atlases.size() == 12, "Twelve isolated source poses including raising and recovery")
	check(ATTACK.frame_atlases[5].resource_path.ends_with("atalanta-north-aim-v3.png"), "Correct left-hand rear aim source")
	check(ATTACK.frame_atlases[6].resource_path.ends_with("atalanta-north-release-v3.png"), "Correct release with repaired bowstring")
	...
	check(is_equal_approx(ATTACK.strike_time, .465), "Release marker follows authored raising poses")
	for i in range(12):
		if i > 0: actor.advance_visual(ATTACK.durations[i-1])
		check(actor.clip == ATTACK and actor.texture.atlas == ATTACK.frame_atlases[i], "Actual chronological pose selected %s" % i)
