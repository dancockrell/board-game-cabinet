...
func _initialize():
	...
	check(ATTACK.frame_atlases.size() == 5, "Five isolated source poses including three recoveries")
	...
	check(is_equal_approx(ATTACK.strike_time, .09), "Release marker remains on second pose")
	for i in range(2,5):
		actor.advance_visual(.075)
		check(actor.clip == ATTACK and actor.texture.atlas == ATTACK.frame_atlases[i], "Actual recovery source selected %s" % i)
