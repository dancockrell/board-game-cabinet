...
func _initialize():
	...
	check(ATTACK.frame_atlases.size() == 8, "Eight isolated source poses including raising and recovery")
	check(ATTACK.frame_atlases[3].resource_path.ends_with("atalanta-north-aim-v3.png"), "Correct left-hand rear aim source")
	check(ATTACK.frame_atlases[4].resource_path.ends_with("atalanta-north-release-v3.png"), "Correct release with repaired bowstring")
	...
	check(is_equal_approx(ATTACK.strike_time, .315), "Release marker follows authored raising poses")
	for i in range(8):
