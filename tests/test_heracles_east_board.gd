...
func run():
	...
	if seen.size()!=4: push_error("East gait did not play all four source poses"); quit(1); return
	state.elapsed+=.05
	board.show_state(state,.05)
	if actor.clip!=board.HERACLES_REST.east: push_error("Stopped archer failed to recover east rest"); quit(1); return
	board.free()
	print("Heracles east board: four source poses, authoritative movement and stopped recovery passed")
	quit(0)
