extends SceneTree
const Walk=preload("res://themes/hydra_south_walk.tres")
...
func run():
	...
	if DisplayServer.get_name()=="headless": print("Hydra south walk: eight distinct cells, ordered timing and loop passed"); quit(0); return
