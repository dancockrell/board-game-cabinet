...
func _initialize() -> void:
	...
	check(ThrustClip.validation_error()=="", "Rich thrust frame contract is valid")
	var invalid=ThrustClip.duplicate()
	invalid.regions = invalid.regions.duplicate()
	invalid.regions[0] = Rect2i(0,0,361,362)
	...
	if DisplayServer.get_name() != "headless":
		...
		actor.set_damage_flash(1.0)
		actor.play_attack(500,ThrustClip)
		check(actor.clip==ThrustClip and actor.damage_flash==1.0 and actor.material_override.get_shader_parameter("damage_flash")==1.0, "Damage highlight survives attack clip change without replacing pose")
		actor.reset_playback(rest)
		check(actor.damage_flash==0.0, "Rematch clears damage highlight")
		actor.free()
