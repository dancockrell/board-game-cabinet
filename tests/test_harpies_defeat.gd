...
const CLIPS=[preload("res://themes/harpies_south_defeat.tres"),preload("res://themes/harpies_north_defeat.tres")]
var checks:=0
var failures:=0
...
func run():
    for clip in CLIPS:
        ...
        for i in 8:
            ...
            check(is_equal_approx(clip.frame_pivots[i].y,clip.regions[i].size.y-2),"Ground anchor remains two pixels above crop bottom")
            actor.show_time(elapsed+.001)
        ...
        check(is_equal_approx(clip.draw_scale,1.9),"Native rest-comparison scale retained")
        actor.free()
    print("Harpy defeat Resources: %d checks, %d failures"%[checks,failures])
