extends SceneTree
const Walk=preload("res://themes/atalanta_west_walk.tres")
...
func run():
	...
	if seen.size()!=8: push_error("West gait did not play all eight source poses"); quit(1); return
	state.elapsed+=.05
	board.show_state(state,.05)
	if actor.clip!=board.ATALANTA_REST.west: push_error("Stopped archer failed to recover west rest"); quit(1); return
	board.free()
	print("Atalanta west board: eight source poses, authoritative movement and stopped recovery passed")
	quit(0)
