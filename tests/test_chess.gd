...
func _init() -> void:
	...
	expect(perft(kiwipete, 3) == 97862, "Kiwipete depth 3")
	...
	var pinned = State.from_fen("k3r3/8/8/8/8/8/4R3/4K3 w - - 0 1")
	expect("e2f2" not in moves(pinned) and "e2e8" in moves(pinned), "Pinned rook stays on pin line")
	var pawn_attack = State.from_fen("4k3/8/8/8/8/3p4/8/4K3 w - - 0 1")
	expect("e1e2" not in moves(pawn_attack), "King cannot enter pawn attack")
	var adjacent = State.from_fen("8/8/8/8/8/4k3/8/4K3 w - - 0 1")
	expect("e1e2" not in moves(adjacent), "Kings remain separated")
	var no_capture_king = State.from_fen("4k3/8/8/8/8/8/4R3/4K3 w - - 0 1")
	expect("e2e8" not in moves(no_capture_king), "King capture is never generated")
	var returned_rook = play(play(play(castle, "h1h2"), "h8h7"), "h2h1")
	expect("K" not in returned_rook.castling, "Returning rook cannot restore castling")
	var phantom_ep = play(initial, "e2e4")
	var no_ep = phantom_ep.copy()
	no_ep.en_passant = -1
	expect(Rules.position_key(phantom_ep) == Rules.position_key(no_ep), "Unusable en passant ignored in repetition")
	no_ep = ep.copy()
	no_ep.en_passant = -1
	expect(Rules.position_key(ep) != Rules.position_key(no_ep), "Usable en passant changes repetition identity")
	no_ep = pinned_ep.copy()
	no_ep.en_passant = -1
	expect(Rules.position_key(pinned_ep) == Rules.position_key(no_ep), "Pinned en passant ignored in repetition")
	expect(Rules.san(initial, {"from": 12, "to": 28, "promotion": ""}) == "e4", "Pawn SAN")
	expect(Rules.san(castle, {"from": 4, "to": 6, "promotion": ""}) == "O-O", "Castling SAN")
	expect(Rules.san(ep, {"from": 36, "to": 43, "promotion": ""}) == "exd6", "En passant SAN")
	var knights = State.from_fen("4k3/8/8/8/8/8/8/1N2KN2 w - - 0 1")
	expect(Rules.san(knights, {"from": 1, "to": 11, "promotion": ""}) == "Nbd2", "SAN disambiguates matching pieces")
	var premate = initial
	for notation in ["f2f3", "e7e5", "g2g4"]:
		premate = play(premate, notation)
	expect(Rules.san(premate, {"from": 59, "to": 31, "promotion": ""}) == "Qh4#", "Mate SAN")
